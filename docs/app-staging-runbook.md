...
For 0.3.7 and later, set `Environment=BITCOINWALK_SERVER_READ_RELAY=ws://127.0.0.1:3334` in the staging app unit when activating the release. The server-rendered paid/free event pages and city redirects then read the existing local organizer relay instead of consuming the public IP's WebSocket connection budget. The value is runtime-only, never included in event relay hints or browser JavaScript. The public `NEXT_PUBLIC_READ_RELAYS` remains `wss://relay-staging.bitcoinwalk.org`; do not replace it with a loopback URL. Verify `http://127.0.0.1:3334/healthz` before activation and confirm the app's sandbox can open a local WebSocket. On rollback to a release before 0.3.7, remove this additional unit setting along with restoring the previous `WorkingDirectory`.
