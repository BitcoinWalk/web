...
3. Package the contents of `.next/standalone/` as the release root, plus `.next/static/` under that root's `.next/` directory (and `public/` if the source contains one). Confirm `server.js`, `package.json`, `node_modules/`, `.next/server/`, and `.next/static/` exist. This reproduces the observed standalone layout; this repository has no app release packager or installer yet.
4. Transfer the package to a **new** root-owned, service-readable version directory under `/opt/bitcoinwalk-app-staging/releases/`. Do not overwrite the running or previous release. Make `.next/cache` a symlink to `/var/cache/bitcoinwalk-app-staging`, as in 0.3.6, so the service can write cache data while `ProtectSystem=strict` keeps the release read-only.
