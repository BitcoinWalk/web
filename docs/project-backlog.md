...
| BW-50 | 4, 7 | Super-admin new-city announcement | ⚪ Planned | When the super-admin approves a new city and its first walk, show a reviewable public Nostr announcement and require a separate super-admin signature before publishing it from the super-admin profile: `New chapter of BitcoinWalk starting in <city>! <country flag> You are welcome to join us on <day> <date> <time>. Bring a friend.` City, local weekday/date/time and country flag must come from the approved city and first-walk data; announcement signing must stop if those values cannot be verified. Publish once per first city approval, retain the signed event for same-signature retry, prevent duplicates after refresh/restart, and expose partial success if city approval succeeds but announcement delivery needs retry. Depends on BW-47 and verified local-time/country resolution. |
