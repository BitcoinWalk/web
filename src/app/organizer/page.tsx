...
import { queryDirectoryRecords, queryAuthorizations, type ApprovalRecord, type CityRevision } from "../../nostr/city-records";
import { archivedCityIds } from "../../nostr/moderation";
import { editableCityRevisions, editedCity, localDateTime } from "../../nostr/organizer-edit";
...
export default function OrganizerPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const available=editableCityRevisions(key,grants,revisions,approvals);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const date = new Date(String(form.get("startAt")));
      if(Number.isNaN(date.getTime())) throw new Error("Choose a valid date and time.");
      const candidate=editedCity(base.city,{startAt:date.toISOString(),description:String(form.get("description")),meetingPoint:{...pin,description:String(form.get("meetingDescription"))},heroImageUrl:String(form.get("heroImageUrl"))});
      const key=await getBrowserExtensionPubkey();
  ...
  return <main>
    <p>BitcoinWalk / organizer</p><h1>Edit your BitcoinWalk</h1>
    <p>Staging relay: {relayConfig.writeRelays.join(", ")}. Pending edits are publicly readable; do not include private details. Your extension keeps your private key.</p>
    <p><Link href="/start">Start a new walk</Link>{" · "}<Link href="/admin">Super-admin approvals</Link></p>
    <p><Link href="/organizer/events">Plan recurring walk events (preview)</Link></p>
    <button type="button" disabled={busy} onClick={load}>Connect and load my walks</button>
    ...
    }}><option value="">Select a walk</option>{cities.map(r=><option key={r.city.cityId} value={r.city.cityId}>{r.city.cityName} — {r.city.cityId}</option>)}</select></label>}
    {base && <section>
      ...
      <form key={base.event.id} onSubmit={submit}>
        <fieldset disabled={busy || submitted}>
          ...
          <label>Walk date and time <input name="startAt" type="datetime-local" defaultValue={localDateTime(base.city.startAt)} required /></label>
          <p>Time is displayed in your device’s local timezone.</p>
