...
export default async function CityOrEventPage({params}:{params:Promise<{city:string}>}) {
  ...
  if(city.startsWith("nevent1")) {
    let result;
    try{result=await resolveCalendarLink(city,relayConfig.readRelays);}catch{return <main><h1>Event temporarily unavailable</h1><p>The relay could not be read. Please try again later.</p></main>;}
  ...
  try{
    ...
    if(event)redirect(`/${encodeURIComponent(city)}/${calendarNevent(event,relayConfig.readRelays)}`);
    return <main><h1>BitcoinWalk {walk.revision.city.cityName}</h1><p>No upcoming walk has been scheduled.</p><p><Link href="/">Browse BitcoinWalks</Link></p></main>;
  }catch(error){
