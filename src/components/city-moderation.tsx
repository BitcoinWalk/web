...
export default function CityModeration(){
 ...
 const lock=useRef(false);const [busy,setBusy]=useState(false),[rows,setRows]=useState<ManagedCity[]>([]),[events,setEvents]=useState<Event[]>([]),[message,setMessage]=useState("Load cities to manage approvals, occurrences and archived cities.");
 ...
 async function reload(){const r=await records();const e=await queryCalendarEvents(relayConfig.writeRelays);setRows(r);setEvents(e);}
 ...
 async function decide(row:ManagedCity,action:"approve"|"disapprove"|"archive"){
  await run(async()=>{const city=row.revision.city;await current(row);
   ...
   await reload();setMessage(`${city.cityName}: ${saved.state}. Confirmed by relay read-back. No relay infrastructure was deleted.`);
  });
 }
 async function removeEvent(row:ManagedCity,item:ManagedCalendarEvent){await run(async()=>{
  for(const relay of relayConfig.writeRelays){const endpoint=new URL(relay);endpoint.protocol="https:";const response=await fetch(endpoint,{headers:{Accept:"application/nostr+json"},signal:AbortSignal.timeout(5000),cache:"no-store"});if(!response.ok||(await response.json()).version!=="bitcoinwalk-organizers-0.6.0")throw new Error("Install and verify organizer occurrence relay 0.6.0 before deleting an event. Nothing signed or deleted.");}
